...
public class EditalController {
    ...
    public List<EditalDTO> listarEditais(
        @RequestHeader("Authorization") String auth){ 
        String token = auth.replace("Bearer ", "");
        return service.listarEditais(token);
    }
    
    @PostMapping("{id}/lances")
    public String registroLance (@RequestHeader("Authorization") String auth, @RequestBody LanceDTO lance, @PathVariable Long id ){
        String token = auth.replace("Beare r", "");
        lanceService.RegistroLance(id, lance, token);
